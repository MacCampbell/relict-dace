...
```{r}
s2<-k2df%>% arrange(`Collection Locality`, `Locality`, Individual) %>% mutate(Index=1:n()) %>% dplyr::select(Label, Index, Locality, `Collection Locality`, Individual, Q1, Q2) %>% gather(key=Ancestry, value=Q, 6:7)
...
k3df$`Collection Locality`<-factor(k3df$`Collection Locality`, levels=order)
...
```{r}
ggarrange(arrangeGrob(fig2a,fig2b + ggtitle("E"), ncol=2, widths=c(1,1)), 
          arrangeGrob(kplot2 + ggtitle("C") + theme(plot.title = element_text(size=16, face="bold", hjust=0)),
                      kplot3 + ggtitle("D") + theme(plot.title = element_text(size=16, face="bold", hjust=0)), ncol=1, heights=c(2,2.7)),
          ncol = 1, nrow = 2, widths = c(1,3), heights=c(10,6))
ggsave("outputs/202/Figure-2-new.pdf", width=8.5, height=11)
ggsave("outputs/202/Figure-2-new.jpg", width=8.5, height=11)
