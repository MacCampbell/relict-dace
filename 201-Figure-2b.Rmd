...
```{r}
t2b<-ggtree(t4, branch.length = 1, layout = "daylight") %<+% keepers
t2b$data$label<-gsub("RD6","",t2b$data$label)
```

```{r}
fig2b <- t2b +
  geom_tippoint(aes(size=`Sample Size`, fill=`Geographic Region`), pch=21, alpha=1.00) +
  geom_text_repel(data=filter(t2b$dat, isTip==TRUE), aes(label=label, x=x, y=y), alpha=0.75, size=3, 
                   segment.color = NA) +
  #geom_tiplab2(aes(label=label, angle=angle), size=2) +
  scale_fill_manual(values=c("blue","red","green")) +
  theme(legend.position = "") +
  ggtitle("C") +
  theme(plot.title = element_text(size=16, face="bold", hjust=0)) +
  theme(plot.margin = margin(0.2,0.0,0,1, "cm")) +
  geom_nodelab(fontface="bold", size=3) 
#  xlim(-10,NA)
